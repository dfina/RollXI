...
> **PROMOTION-COMPLETENESS RULE (must follow for EVERY competition — EC/UCL, CWC, Fairs, UEFA, Europa, Conference, Intertoto).** When a competition is promoted to pickable at "finalist/semi-final" level, that means ALL FOUR semi-final-stage teams of each edition: the winner, the runner-up, AND both losing semi-finalists. Do NOT promote only the losing semi-finalists and leave the two finalists sitting in the tier-O index — that is the exact gap that happened with EC/UCL Wave 1 (1a-1g promoted the losing SFs but left winners + runners-up as opponents until this corrective pass). The finalists are the most iconic, highest-recognisability club-seasons and must be pickable. Concretely, whenever a promotion wave for a competition runs: (1) take winner + runner-up + both SFs of each edition; (2) build full rosters for any not already pickable; (3) remove EVERY promoted club-season from the tier-O index in the same pass; (4) re-run the global O/P de-dup sweep and assert zero overlap. A competition's tier-O index should trend toward EMPTY as its promotion completes. After any wave, audit pickable squads tagged "winner/champion" and "runner-up" vs "semi-finalist" — if winners/runners-up are missing relative to SFs, the wave is incomplete.

> **EC/UCL FINALS PROMOTION (Wave F, in progress):** corrective pass promoting all 134 EC/UCL winner + runner-up club-seasons (1955-2026) to pickable, most-recent-decade-first. Wave Fa = 2015-16..2025-26 finals; Fb = 2004-05..2014-15; Fc = 1990-91..2003-04 (DONE); Fd = 1980-81..1989-90 (DONE); Fe = 1970-71..1979-80 (DONE); Ff = 1955-56..1969-70 (DONE — all 15 editions' winners + runners-up pickable, web-verified, removed from tier-O; EC/UCL tier-O now at ZERO, O/P overlap=0).
>
> **GLOBAL O/P DE-DUP SWEEP (14 Jun 2026).** Audit found steps 3-4 of the promotion-completeness rule (remove promoted club-seasons from tier-O; assert zero overlap) had never run across Waves 1a-1g/Fa/Fb: 168 club-seasons sat in BOTH tiers. Swept the EC/UCL index (284→96 rows) and retired the provisional placeholder (48→1; the B-series indexes have landed, so all covered/promoted rows were removed, keeping only Man City 2018-19 which is unique and in no verified index). CWC/UEFA indexes untouched (no promotion there yet). O/P overlap asserted 0. Going forward every wave MUST remove its promoted rows from tier-O in the same pass.
